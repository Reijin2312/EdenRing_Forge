...
public class GraviliteShardsBlock extends BaseAttachedBlock implements RenderLayerProvider {
	...
	public GraviliteShardsBlock() {
		super(BlockBehaviour.Properties.copy(Blocks.AMETHYST_CLUSTER).lightLevel(state -> 15).noCollission().noOcclusion());
	}
	...
	public UnbakedModel getModelVariant(ResourceLocation stateId, BlockState blockState, Map<ResourceLocation, UnbakedModel> modelCache) {
		ResourceLocation shardsUp = new ResourceLocation(
			stateId.getNamespace(),
			"block/" + stateId.getPath() + "_shards_up"
		);
